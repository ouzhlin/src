#include "core/gesture/GeometricRecognizer.h"
...
Path2D GeometricRecognizer::getTemplatePath2D(string name)
{
    for (int i = 0; i < (int)templates.size(); i++)
    {
        if (templates[i].name.compare(name) == 0 ) {
            return templates[i].points;
        }
    }
    Path2D path;
    return path;
